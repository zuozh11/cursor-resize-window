...
public final class WindowResizeApp: NSObject, NSApplicationDelegate, @unchecked Sendable {
    ...
    fileprivate func handle(_ type: CGEventType, event: CGEvent, proxy: CGEventTapProxy) -> Unmanaged<CGEvent>? {
        ...
        case .leftMouseDragged:
            ...
            if nativeDragState != nil {
                return applyNativeResize(to: event, proxy: proxy)
            }
    ...
    private func applyNativeResize(to event: CGEvent, proxy: CGEventTapProxy) -> Unmanaged<CGEvent>? {
        ...
        if nativeDragState.needsPointerWarp {
            ...
            if let mouseDown = consumedMouseDown {
                mouseDown.flags.remove(.maskControl)
        ...
        if nativeDragState.isMove,
           isFirstDrag || nativeDragState.pendingInitialDrag != nil {
            ...
            if isFirstDrag {
                scheduleInitialMoveDrag(for: nativeDragState)
            }
            return nil
        }
        updateNativeDragFeedback(at: event.location)
    ...
    private func scheduleInitialMoveDrag(for state: NativeDragState) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(20)) { [weak self] in
            ...
            drag.setIntegerValueField(.eventSourceUserData, value: delayedMoveDragMarker)
    ...
    private func activateApplicationIfNeeded(for window: AXUIElement) {
        var pid: pid_t = 0
        guard AXUIElementGetPid(window, &pid) == .success else {
            return
        }
...
private let delayedMoveDragMarker: Int64 = 0x4352_5749
