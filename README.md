...
Then hold `ctrl`, left-click a window, and drag to resize or move it. Movement or resizing starts only after the pointer moves more than 1 macOS point from the press position. Until then, no drag feedback appears and the utility does not activate the window. Releasing within this distance replays the original Ctrl-click to the app; slight pointer jitter remains a click. The press is held until this decision, so app actions that normally happen on mouse-down occur on release. The press position selects the move or resize region, while movement starts from the position where the threshold is crossed, without replaying the initial dead-zone distance. Ctrl-drag remains reserved for window movement and resizing.
