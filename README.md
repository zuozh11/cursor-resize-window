...
Center movement prefers the original mouse-down X coordinate, 6 points below the window's top edge by default. Apps with custom title bars may need a different offset to avoid dragging a tab or clicking a button. The utility first checks the original mouse X position, then uses Accessibility hit testing to avoid controls and choose the nearest title-bar background gap. Background candidates are not guaranteed native draggable regions. If no candidate is found, the center gesture is left untouched. The red dot shows the selected target.
